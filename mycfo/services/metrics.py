...
def compute_metrics(*, workspace: Workspace, transactions: list[Transaction], as_of: date) -> dict:
    ...
    window_transactions = [txn for txn in transactions if start_dt <= txn.occurred_at <= end_dt]
    previous_start = start_dt - timedelta(days=30)
    previous_transactions = [txn for txn in transactions if previous_start <= txn.occurred_at < start_dt]
...
def _distinct_customers(transactions: list[Transaction]) -> int:
    customers = {txn.customer_ref for txn in transactions if txn.customer_ref}
